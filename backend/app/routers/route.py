...
_INVALID_PLACE_PATTERN = re.compile(r"(--|;|/\*|\*/|['\"`])")


def _clean_place_param(value: str, field_name: str) -> str:
    cleaned = value.strip()
    if not cleaned:
        raise HTTPException(status_code=422, detail=f"{field_name} 不能为空")
    if _INVALID_PLACE_PATTERN.search(cleaned):
...
async def route_night_safe(
    origin: str = QParam(..., min_length=1, max_length=100, description="出发地名称"),
    destination: str = QParam(..., min_length=1, max_length=100, description="目的地名称"),
    ...
    """
    origin = _clean_place_param(origin, "origin")
    destination = _clean_place_param(destination, "destination")
...
async def route_evacuation(
    origin: str = QParam(..., min_length=1, max_length=100, description="出发地名称"),
    destination: str = QParam("最近安全集结点", min_length=1, max_length=100, description="目的地，默认为最近操场或校门"),
    user_lng: float | None = QParam(None, description="用户当前经度"),
    user_lat: float | None = QParam(None, description="用户当前纬度"),
) -> RouteResult:
    """
...
async def route_multistop(req: MultiStopRouteRequest) -> RouteResult:
    ...
    """
    origin = _clean_place_param(req.origin, "origin")
    destination = _clean_place_param(req.destination, "destination")
    stops = [_clean_place_param(stop, "stops") for stop in req.stops]
