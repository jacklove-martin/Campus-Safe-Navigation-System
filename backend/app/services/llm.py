...
async def parse_user_query(text: str) -> ParsedTask:
    ...
    try:
        ...
        return ParsedTask(raw_text=text, **data)
...
_KEYWORD_INTENT: list[tuple[list[str], IntentType, RouteMode | None]] = [
    ...
    (["食堂", "便利店", "宿舍", "多目标", "顺路"],      IntentType.multi_stop_route,   RouteMode.multi),
    (["在哪", "怎么走", "导航", "路线", "去"],          IntentType.navigation,         RouteMode.night),
    (["营业", "开门", "几点", "有没有", "卖"],          IntentType.facility_query,     None),
]
...
def _fallback_parse(text: str) -> ParsedTask:
    ...
    if "文具" in text or "笔记本" in text:
        priority_rule = "stationery"
    elif "夜宵" in text or "夜间" in text:
